...
async def import_excel(
    ...
    for rec in result["to_create"]:
        try:
            ...
            rec["site_id"] = site_id
            cell = Cell5G(
                **{k: v for k, v in rec.items() if hasattr(Cell5G, k)},
                created_by=current_user.id,
            )
            db.add(cell)
            db.flush()
            record_cell5g_revision(
...
def create_cell(
    ...
    if not db.query(Site).filter(Site.id == payload.site_id).first():
        raise HTTPException(status_code=400,
                            detail=f"Site id={payload.site_id} not found.")
...
def update_cell(
    ...
    # Capture snapshot BEFORE applying any changes
    ...
    # flush so the revision snapshot reads the NEW values from the ORM object
    db.flush()
