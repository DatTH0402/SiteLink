...
def _fmt_rev(rev, extra: dict = None) -> dict:
    base = {
        ...
        "changed_fields":   json.loads(rev.changed_fields) if rev.changed_fields else {},
...
def list_site_revisions(
    ...
    if site_name:
        # search across all known names (current + old ref)
        q = q.filter(
...
def list_cell3g_revisions(
    ...
    rows = q.order_by(desc(Cell3GRevision.created_at)).offset(skip).limit(limit).all()
    return [_fmt_rev(r, {
        "cell_id_ref": r.cell_id_ref, "site_id": r.site_id,
        ...
        "loai_anten": r.loai_anten, "chung_anten": r.chung_anten,
        "baseband": r.baseband, "rf": r.rf, "cell_id": r.cell_id,
        "arfcn": r.arfcn, "psc": r.psc, "mimo": r.mimo,
    }) for r in rows]
...
def list_cell4g_revisions(
    ...
    rows = q.order_by(desc(Cell4GRevision.created_at)).offset(skip).limit(limit).all()
    return [_fmt_rev(r, {
        "cell_id_ref": r.cell_id_ref, "site_id": r.site_id,
        "site_name": r.site_name, "site_name_old": r.site_name_old,
        "cell_name": r.cell_name, "cell_name_old": r.cell_name_old,
        ...
        "loai_anten": r.loai_anten, "chung_anten": r.chung_anten,
        "baseband": r.baseband, "rf": r.rf, "cell_id": r.cell_id,
        "earfcn": r.earfcn, "pci": r.pci,
        "root_sequence_id": r.root_sequence_id, "mimo": r.mimo,
    }) for r in rows]
...
def list_cell5g_revisions(
    ...
    rows = q.order_by(desc(Cell5GRevision.created_at)).offset(skip).limit(limit).all()
    return [_fmt_rev(r, {
        "cell_id_ref": r.cell_id_ref, "site_id": r.site_id,
        "site_name": r.site_name, "site_name_old": r.site_name_old,
        "cell_name": r.cell_name, "cell_name_old": r.cell_name_old,
