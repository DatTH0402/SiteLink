...
def bulk_delete(
    ...
    errors, deleted = [], 0
    for cid in ids:
        cell = db.query(Cell4G).filter(Cell4G.id == cid).first()
        if not cell: errors.append(f"Cell id={cid} not found"); continue
        try:
            db.delete(cell); db.commit()
            log_action(db, current_user, "DELETE", "cells_4g", cid)
            deleted += 1
...
def bulk_update(
    ...
    for cid in ids:
        ...
        if not cell: errors.append(f"Cell id={cid} not found"); continue
        try:
            old_snap = _cell4g_snapshot(cell)
            for k, v in changes.items():
...
async def dry_run_excel(
    ...
    try: result = parse_cell4g_excel(content, db=db, dry_run=True)
...
async def import_excel(
    ...
    content = await file.read()
    try: result = parse_cell4g_excel(content, db=db, dry_run=False)
    except Exception as e: raise HTTPException(status_code=400, detail=f"Cannot read Excel: {e}")
...
def create_cell(
    payload: Cell4GCreate, db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    log_action(db, current_user, "CREATE", "cells_4g", cell.id, new_value=payload.model_dump())
    return cell


@router.put("/{cell_id}", response_model=Cell4GRead)
def update_cell(
    cell_id: int, payload: Cell4GUpdate,
