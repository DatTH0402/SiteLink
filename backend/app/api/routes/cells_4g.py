...
def list_cells(
    ...
    if mien:
        q = q.filter(Cell4G.mien == mien)
    if tinh:
        q = q.filter(Cell4G.tinh == tinh)
    if vendor:
        q = q.filter(Cell4G.vendor == vendor)
    if mimo:
        q = q.filter(Cell4G.mimo == mimo)
    if vung_phu_song:
        q = q.filter(Cell4G.vung_phu_song == vung_phu_song)
